...
def get_current_l(ts, coors, problem, equations=None, mode=None, **kwargs):
    if mode == 'qp':
        ...
        phil_values = ev('ev_surface_integrate.i.Gamma1(phil)',
                         mode='qp', verbose=False)
        phis_values = ev('ev_surface_integrate.i.Gamma2(phis)',
                         mode='qp', verbose=False)
...
fields = {
    'potential_l': ('real', 1, 'Omega1', 1),
    'potential_s': ('real', 1, 'Omega2', 1),
    'current_l': ('real', 1, 'Omega1', 1),
...
equations = {
    'eq1': """dw_laplace.i.Omega1( m.electroyle_conductivity, psil, phil )
           = dw_surface_integrate.i.Gamma1(il0.val,psil)""",
    'eq2': """dw_laplace.i.Omega2( m.electric_conductivity, psis, phis )
           = -dw_surface_integrate.i.Gamma2(is0.val,psis)""",
    # 'eq1': """dw_laplace.i.Omega1( m.electroyle_conductivity, psil, phil  )
    #        = dw_surface_dot.i.Gamma1(psil,il)""",
    # 'eq2': """dw_laplace.i.Omega2( m.electric_conductivity, psis, phis )
    #        = dw_surface_dot.i.Gamma2(psis,is)""",
    'eq3': """dw_volume_dot.i.Omega1(ils, il )
               = dw_diffusion_coupling.i.Omega1(m.electroyle_conductivity,phil,ils)""",
...
def postproc(out, pb, state, extend=False):
    ...
    currentl = ev('ev_diffusion_velocity.i.Omega1(m.electroyle_conductivity,phil)', mode='qp')
    currents = ev('ev_diffusion_velocity.i.Omega2(m.electric_conductivity,phis)', mode='qp')
    phil = ev('ev_surface_integrate.i.Gamma1(phil)', mode='qp')
    phis = ev('ev_surface_integrate.i.Gamma2(phis)', mode='qp')
    ...
    out['currents'] = Struct(name='output_data', mode='vertex',
                             data=currents.reshape(currents.shape[0] * currents.shape[1], 1), dofs=None)
